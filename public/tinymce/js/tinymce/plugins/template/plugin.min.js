...
!(function () {
    ...
    function i(e) {
        return (n = typeof (t = e)), 'string' == (null === t ? 'null' : 'object' == n && (Array.prototype.isPrototypeOf(t) || (t.constructor && 'Array' === t.constructor.name)) ? 'array' : 'object' == n && (String.prototype.isPrototypeOf(t) || (t.constructor && 'String' === t.constructor.name)) ? 'string' : n)
        var t, n
    }
    var t,
    ...
    function p(e, t, n) {
        ...
        return (t = (t = (t = (t = (t = (t = (t = (t = (t = (t = (t = (t = (t = (t = (t = t.replace('%D', '%m/%d/%Y')).replace('%r', '%I:%M:%S %p')).replace('%Y', '' + n.getFullYear())).replace('%y', '' + n.getYear())).replace('%m', f(n.getMonth() + 1, 2))).replace('%d', f(n.getDate(), 2))).replace('%H', '' + f(n.getHours(), 2))).replace('%M', '' + f(n.getMinutes(), 2))).replace(
            '%S',
            '' + f(n.getSeconds(), 2)
        )).replace('%I', '' + (((n.getHours() + 11) % 12) + 1))).replace('%p', n.getHours() < 12 ? 'AM' : 'PM')).replace('%B', '' + e.translate(c[n.getMonth()]))).replace('%b', '' + e.translate(o[n.getMonth()]))).replace('%A', '' + e.translate(a[n.getDay()]))).replace('%a', '' + e.translate(r[n.getDay()]))).replace('%%', '%')
    }
    function x(n, e) {
        return (
            ...
            if (t && 0 !== t.length)
                return P.from(
                    O.map(t, function (e, t) {
                        ...
                        return { selected: 0 === t, text: e.title, value: { url: n(e) ? P.from(e.url) : P.none(), content: n(e) ? P.none() : P.from(e.content), description: e.description } }
                    })
                )
                    ...
                    var r,
                        ...
                        y =
                            ...
                            -1 === (a = n).indexOf('<html>') &&
                                ...
                                (l = r.getParam('content_css_cors', !1, 'boolean') ? ' crossorigin="anonymous"' : ''),
                                ...
                                (i = -1 === (g = (p = r).getParam('body_class', '', 'string')).indexOf('=') ? g : (d = p).getParam('body_class', '', 'hash')[d.id] || ''),
                                (u = r.dom.encode),
                                (s = '<script>document.addEventListener && document.addEventListener("click", function(e) {for (var elm = e.target; elm; elm = elm.parentNode) {if (elm.nodeName === "A" && !(' + (S.mac ? 'e.metaKey' : 'e.ctrlKey && !e.altKey') + ')) {e.preventDefault();}}}, false);</script> '),
                                (f = (m = r.getBody().dir) ? ' dir="' + u(m) + '"' : ''),
                                (a = '<!DOCTYPE html><html><head><base href="' + u(r.documentBaseURI.getURI()) + '">' + o + s + '</head><body class="' + u(i) + '"' + f + '>' + a + '</body></html>')),
    ...
    n.add('template', function (e) {
        ...
        var n, r, a, o, c, l
        ;(n = e).ui.registry.addButton('template', { icon: 'template', tooltip: 'Insert template', onAction: t }),
            n.ui.registry.addMenuItem('template', { icon: 'template', text: 'Insert template...', onAction: t }),
            (a = e).addCommand(
                'mceInsertTemplate',
                (function (r) {
